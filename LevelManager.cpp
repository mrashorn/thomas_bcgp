...
int** LevelManager::nextLevel(VertexArray& rVaLevel)
{
	...
	m_LevelSize.x = s.length(); // all rows must be same length
	
	// Go back to the start of the file
	inputFile.clear();
	inputFile.seekg(0, ios::beg); // move file cursor to beginning of file

	// Prepare the 2D array to hold the int values from the file
	int** arrayLevel = new int* [m_LevelSize.y];
	for (int i = 0; i < m_LevelSize.y; ++i)
	{
		// Add an empty new array into each array element
	...
	while (inputFile >> row)
	{
		for (int x = 0; x < row.length(); x++)
		{
			...
			arrayLevel[y][x] = atoi(&val); // atio converts char to int
		}
		y++;
	}
